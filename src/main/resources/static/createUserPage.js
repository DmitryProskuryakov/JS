...
async function addNewUser(event) {
    ...
    try {
        ...
        var triggerTabList = [].slice.call(document.querySelectorAll('#list-home'))
        ...
        var triggerEl = document.querySelector('#list-home[href="#home"]');
        if (triggerEl) {
...
async function getRoles() {
    try {
        const response = await fetch('/app/admin/getRoles');
        if (!response.ok) {
            throw new Error(`Server returned ${response.status} ${response.statusText}`);
        }
